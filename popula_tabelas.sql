...
BEGIN
    -- Buscar o REF da avaliação recém-criada
    SELECT REF(a) INTO v_avaliacao_ref_1 FROM avaliacao_obj_tab a WHERE a.id_avaliacao = 1;

    -- Inserir um novo consumo de conteúdo
    INSERT INTO perfil_consome_conteudo_obj_tab
    VALUES (
        perfil_consome_conteudo_t(
            1,                      -- id_conteudo
            1,                       -- id_perfil
            'jose.silva@email.com',      -- email
            TO_TIMESTAMP('01/02/2025 14:30:00', 'DD/MM/YYYY HH24:MI:SS'),             -- data_hora
            'Celular',             -- dispositivo_utilizado
            75,                       -- progresso_percentual
            v_avaliacao_ref_1           -- avaliacao REF
        )
    );
END;
/
